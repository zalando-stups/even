...
def grant_ssh_access(args):
    ...
    forced_command = 'echo {}; /bin/bash'.format(WELCOME_MESSAGE.format(date=now.strftime('%Y-%m-%d %H:%M:%S')))
...
def revoke_ssh_access(args: list):
    ...
    now = datetime.datetime.now()
    forced_command = 'echo {}'.format(REVOKED_MESSAGE.format(date=now.strftime('%Y-%m-%d %H:%M:%S')))
    generate_authorized_keys(user_name, keys_file, pubkey, forced_command)
